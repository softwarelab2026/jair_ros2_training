...
class BallTracker:
    ...
    def _normalize_ball_pos(self, frame_height: int, frame_width:int, ball_pos_x: int, ball_pos_y: int) -> tuple[float, float]:
    ...
    def _turtle_error(self, norm_ball_pos: tuple[float, float],
                             norm_turtle_pos:tuple[float, float, float]) -> float:
    ...
    def track_ball(self, _frame: Image, turtle_pos: Pose) -> tuple[float, float]:
        ...
        (turtle_linear_err, turtle_angular_err) = self._turtle_error((norm_ball_x, norm_ball_y),
        ...
        self._logger.info(f'turtle pos: {round(norm_turtle_x, 2)}\t,{round(norm_turtle_y, 2)}\ttheta: {round(turtle_theta, 2)}')
